...
TEST(UtilProperties, Basic) {
    PropertyMap map = loadProperties(smack::test::makeResourcePath("smack_properties.props"));
    ASSERT_EQ( "8", map["propertyCount"] );
}

TEST(UtilProperties, BasicObject) {
    Properties map{ smack::test::makeResourcePath("smack_properties.props") };
...
TEST(UtilProperties, ContinuationLines) {
    Properties map{ smack::test::makeResourcePath("smack_properties_continuation.props") };
    ASSERT_TRUE(map.fileFound());
